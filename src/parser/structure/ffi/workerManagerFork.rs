...
// Это не рабочий код, я оставил его тут специально, потому что я делал замеры для него;
// По скорости `fork()` без exec здесь - медленнее чем `std::process::Command` и менее безопаснее.
...
/// Выполняет FFI-функцию в изолированном процессе (fork без exec);
///
...
pub fn callExternal(
  library_path: &str,
  method_name: &str,
  parameters_tokens: &mut [Token],
  result_type: StructureType,
) -> Result<FFIValue, String>
{
  // 1. Преобразуем токены в FFIValue
  let args: Vec<FFIValue> = parameters_tokens
    .iter_mut()
    .map(FFIValue::try_from)
    .collect::<Result<Vec<_>, _>>()?;

  // 2. Преобразуем тип результата
  let ffiResultType: FFIType = FFIType::try_from(result_type)?;

  // 3. Подготавливаем данные для передачи в замыкание (копируем)
  let libraryPath: String = library_path.to_string();
  let functionName: String = method_name.to_string();

  // 4. Используем mem-isolate для выполнения в дочернем процессе
  let result: Result<Result<FFIValue, String>, MemIsolateError> =
    mem_isolate::execute_in_isolated_process(move || {
      // ---- этот код выполняется в дочернем процессе после fork ----
      // (все ресурсы будут автоматически освобождены при завершении)
  ...
  // Обрабатываем результат execute_in_isolated_process
  match result {
    Ok(ok) => Ok(ok?),
    Err(e) => Err(format!("Isolate error: {}", e)),
  }
}